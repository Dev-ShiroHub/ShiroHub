...
local Settings = shirolib.Settings
local Flags = shirolib.Flags

local SetProps, SetChildren, InsertTheme, Create do
	InsertTheme = function(Instance, Type)
		table.insert(shirolib.Instances, {
			Instance = Instance,
			Type = Type
		})
		return Instance
	...
	local function Save(file)
		if readfile and isfile and isfile(file) then
			...
			if type(decode) == "table" then
				if rawget(decode, "UISize") then shirolib.Save["UISize"] = decode["UISize"] end
				if rawget(decode, "TabSize") then shirolib.Save["TabSize"] = decode["TabSize"] end
				if rawget(decode, "Theme") and VerifyTheme(decode["Theme"]) then shirolib.Save["Theme"] = decode["Theme"] end
			end
		end
	end
	
	pcall(Save, "shiro library V5.json")
end
...
-- /////////// --
function shirolib:GetIcon(index)
...
function shirolib:SetTheme(NewTheme)
	if not VerifyTheme(NewTheme) then return end
	
	redzlib.Save.Theme = NewTheme
	SaveJson("shiro library V5.json", shirolib.Save)
...
function shirolib:SetScale(NewScale)
	NewScale = ViewportSize.Y / math.clamp(NewScale, 300, 2000)
	UIScale, ScreenGui.Scale.Scale = NewScale, NewScale
end

function redzlib:MakeWindow(Configs)
	local WTitle = Configs[1] or Configs.Name or Configs.Title or "shiro Library V5"
	local WMiniText = Configs[2] or Configs.SubTitle or "by : TenshiDev"
	...
	local UISizeX, UISizeY = unpack(shirolib.Save.UISize)
	...
	function Window:SelectTab(TabSelect)
		if type(TabSelect) == "number" then
			...
			for _,Tab in pairs(shirolib.Tabs) do
	...
	function Window:MakeTab(paste, Configs)
		...
		FirstTab = true
		local Tab = {}
		table.insert(shirolib.Tabs, {TabInfo = {Name = TName, Icon = TIcon}, func = Tab, Cont = Container})
		Tab.Cont = Container
		...
		function Tab:AddSection(Configs)
			...
			local Section = {}
			table.insert(shirolib.Options, {type = "Section", Name = SectionName, func = Section})
